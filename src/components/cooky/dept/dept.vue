<template>
  <div class="frame-page h-panel">
    ...
    <div class="h-panel-body">
      <Row>
        <Cell :width="12">
          <DropdownMenu button class="h-btn-primary" @click="trigger" :datas="drowList" v-has-perms="'dept:add'">新增</DropdownMenu>
          <Button color="red" @click="handleDel" v-has-perms="'dept:del'">{{isDeleteMode?'删除选中的节点':'删除'}}</Button>
          <Button color="green" v-if="isDeleteMode" @click="isDeleteMode = false && this.$refs.deptTree.updateChoose([])">取消选择</Button>
          <Skeleton active loading v-if="!param.datas"></Skeleton>
          <Tree :option="param" ref="deptTree" :toggleOnSelect="false" @select="onDeptTreeSelect"  :multiple="isDeleteMode" choose-mode="independent"   v-else style="margin-top: 8px;"></Tree>
        </Cell>
        <Cell :width="12">
          <Form
            ref="deptForm"
